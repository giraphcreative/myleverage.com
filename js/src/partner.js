...
jQuery(document).ready(function($){
	...
	if ( $( '.partner-showcase' ) ) {
		...
		var thumbs = $( '.partner-showcase' );
		var thumb_list = thumbs.find( '.partner-list' );

		// we start on page one
		var page = 1;
		
		if ( thumb_list.length && thumb_list.find( 'img' ).length ) {

			// store product nav buttons
			var thumb_nav = thumbs.find( 'button.partner-nav' );
			var thumb_nav_next = thumbs.find( 'button.partner-nav.next' );
			var thumb_nav_previous = thumbs.find( 'button.partner-nav.previous' );

			// count total thumbs
			var total_thumbs = thumb_list.find( 'img' ).length;
